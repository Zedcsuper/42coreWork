...
void free_table(t_din *din_table)
{
    if (din_table)
    {
        if (din_table->forks)
        {
            for (int i = 0; i < din_table->nop; i++)
...
void	*mr_mayhem(void *data)
{
	...
	while (philo->din_table->death)
	{
		...
		{
			pthread_mutex_lock(&philo->din_table->death_mutex);
			print_status(philo->din_table, philo->pid, "died\n");
			philo->din_table->death = 0;
			pthread_mutex_unlock(&philo->din_table->death_mutex);
		}
